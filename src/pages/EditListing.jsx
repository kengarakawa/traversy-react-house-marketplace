...
const EditListing = () => {
  ...
  const onSubmit = async (e) => {
    ...
    if (geolocationEnabled) {
      ...
      if (location.includes("undefined")) {
        setLoading(false)
        toast.error("Cannot find the specific place, please try again")
        return false
      }
    } else {
      geolocation = {
    ...
    // handling uploaded images, saved into firebase storage
    ...
    try {
      ...
      console.log(error)
      setLoading(false)
      toast.error("Error while updating doc")
    }
  }
  ...
  // check if islaready logged in or is the listing owner
  useEffect(() => {
    if( listing && listing.userRef !== auth.currentUser.uid) {
        navigate('/')
        toast.error('Unahthorized owner')        
    }
  })
  


 // fetch listing info into form
  useEffect(() => {
    setLoading(true)
